...
class Customers extends Allow
{
    ...
    public function postInsert()
    {
        ...
        $bus_file = request()->file('bus_file');

        if ($bus_file != null) {


            $info = $bus_file->move($this->DOC_PATH() . $comp_id . DS . $customer['cust_no'], '');
            if ($info) {
                $customer['bus_file'] = $info->getFilename();
            } else {
                // 上传失败获取错误信息
                throw new Exception($bus_file->getError());
            }
        }
